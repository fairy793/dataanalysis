...
#"缺失值处理"
# 缺失值处理有两种方案，一种是分析含缺失值的特征对任务有没有用，没用的特征直接删除，有用的特征依据缺失量，少则删除样本，
# 多则用mean,median或mod补全；另一种方案是分析这些缺失值缺失的原因，并用一定方法将其转换为一类数据（成为
# 类型变量的一个类型）。
...
df_tr = pd.read_csv("./data/train.csv").drop('Id',axis=1)
